...
$zl_sql = "select id,room,time,sth,Contact from LostAndFound where found=1 and dele = 0 order  by id desc ";
$zl_result = mysqli_query($link, $zl_sql);
$zl_info = mysqli_num_rows($zl_result);
var_dump($zl_info);
die;
if ($zl_info > 0) {
	echo '{"status":true ,"code":0,"msg":"", "data":' . json_encode(mysqli_fetch_all($zl_result, MYSQLI_ASSOC), JSON_UNESCAPED_UNICODE) . '}';
